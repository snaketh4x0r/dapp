...
function uploadAndSellFile(path, metadata, price, priceAddress) {
  return async (dispatch, getState) => {
    ...
    try {
      ...
      await daemonService.uploadFile(bucket, path).catch(error => {
...
function createSubscription(amount, paymentAsset, days, seller) {
  return async (dispatch, getState) => {
    ...
    try {
      const {account, market, web3} = getState().web3;
      const erc20 = await new web3.eth.Contract(IERC20['abi'], paymentAsset, {
        from: account,
      });
      console.log('approving market');
      data = await erc20Service.approve(erc20, config.marketAddress, amount);
      if (data.error) {
        throw 'Could not approve Market to transfer funds';
      }
      ...
      dispatch(failure(e));
      dispatch(
        alertActions.error('Error Creating subscription: ' + e.toString()),
      );
      return;
    }
    if (!data.error) {
...
function cancelSubscription(seller) {
  return async (dispatch, getState) => {
    ...
    try {
      ...
      dispatch(failure(e));
      dispatch(
        alertActions.error('Error Cancelling Subscription: ' + e.toString()),
      );
      return;
    }
    if (!data.error) {
      ...
      dispatch(failure(data.error));
      dispatch(
        alertActions.error('Error Cancelling Subscription: ' + data.error),
      );
    }
  };
}
...
function updateSubscriptionInfo(amount, days, asset) {
  return async (dispatch, getState) => {
    ...
    try {
      ...
      console.log(e);
      dispatch(failure(e));
      dispatch(alertActions.error('Error enabling subs: ' + e.toString()));
      return;
    }
    if (!data.error) {
...
function disableSubscriptionInfo() {
  return async (dispatch, getState) => {
    ...
    try {
      ...
      console.log(e);
      dispatch(failure(e));
      dispatch(alertActions.error('Error disabling subs: ' + e.toString()));
      return;
    }
    if (!data.error) {
      dispatch(alertActions.success('Successfully disabled Subscription'));
      dispatch(done());
    } else {
      dispatch(failure(data.error));
      dispatch(
        alertActions.error('Error disabling Subscription: ' + data.error),
      );
    }
  };
}
