      ...
      {allSubscriptions &&
        allSubscriptions.reverse().map((subscription, index) => (
          <div className="subscriptionItem" key={index.toString()}>
            {!seller && (
              <>
                <span className="subscriptionLabel topLabel">Seller</span>
                <div
                  className="seller"
                  onClick={() => {
                    history.push(`/users/${subscription.seller.address}`);
                  }}
                >
                  <ProfileHover
            ...
            {seller && (
              <>
                <span className="subscriptionLabel topLabel">Subscriber</span>
                <div
                  className="subscriber"
                  onClick={() => {
                    history.push(`/users/${subscription.subscriber.address}`);
                  }}
                >
                  <ProfileHover
            ...
            <span>{new Date(subscription.startTime * 1000).toString()}</span>
            ...
            <span className="isExpired">{`${new Date(subscription.stopTime) <
              new Date()}`}</span>
            ...
            <span className="subscriptionLabel">Remaining Balance</span>
            <span className="remainingBalance">{`${(
              subscription.remainingBalance /
              10 ** 18
            ).toFixed(3)} ${getTokenSymbol(subscription.tokenAddress)} (${
              subscription.remainingBalance
            })`}</span>
            {seller && (
              <a
                ...
                onClick={async () => {
                  let amount = await getStreamBalance(subscription.streamId);
                  withdrawFromSubscription(subscription.streamId, amount);
                }}
              >
                Withdraw Balance
              </a>
            ...
            {!seller && (
              <a
                className="subscriptionButton button"
                onClick={() => {
                  cancelSubscription(subscription.seller.address);
                }}
              >
                Cancel Subscription
              </a>
