          ...
          <>
            ...
            <input
              ...
              onChange={e => {
                ...
                if (e.target.value) {
                  setAmount(
                    window.BigInt(seller.amountPerDay) *
                      window.BigInt(e.target.value),
                  );
                }
              }}
            />
            ...
            <p>
              {`Total Amount: ${(Number(amount) / 10 ** 18).toFixed(
                2,
              )} ${getTokenSymbol(seller.tokenAddress)} (${amount})`}
            </p>

            <a
              className="button"
              onClick={async e => {
                if (Number(days) >= Number(seller.minDurationInDays)) {
                  createSubscription(
                    amount,
                    seller.tokenAddress,
                    days,
