...
const UpdateSubscription = ({
  ...
  return (
    <div className="updateSubscriptionInfo">
      {subscription && (
        <>
          <span className="profileTitle">Update Subscription Info</span>
          <p>
            {`Subscription: 
          ...
          {subscription.isEnabled && (
            <>
              ...
              <span>{`Min Days: ${subscription.minDurationInDays}`}</span>
              <span>
                {`Amount Per Day: ${(
          ...
          <div className="amountInput">
            <input
              ...
              onChange={e => {
                ...
                let actualAmount =
                  window.BigInt(e.target.value) * window.BigInt(10 ** 18);
                actualAmount = actualAmount / window.BigInt(86400);
                actualAmount = actualAmount * window.BigInt(86400);
          ...
          <p>
            {`Actual Amount: ${(Number(actualAmount) / 10 ** 18).toFixed(
              2,
            )} ${getTokenSymbol(asset)} (${actualAmount})`}
          </p>

          <a
            className="subscriptionButton button"
            onClick={async e => {
              await updateSubscriptionInfo(actualAmount, days, asset);
              getSubscriptionInfo();
            }}
          >
