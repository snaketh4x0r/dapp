      ...
      if (res.data.user) {
        ...
        if (user && user.subscriptions && user.subscriptions.length) {
          user.subscriptions.some(subscription => {
            if (subscription.seller.address === address.toLowerCase()) {
          ...
          <div className="mainContent">
            <div className="profileLeftBar">
              <div className="profileImage">
                {profile.image && <img src={getImageUrl(profile.image)} />}
              </div>
              <div className="name">
                {profile.emoji
              ...
              <div className="address">{getAccountString(address)}</div>
              <div className="description">{profile.description}</div>
              <a
                className="boxProfile profileButton"
                href={`https://3box.io/${address}${isLoggedInUser && '/edit'}`}
              >
                {isLoggedInUser ? 'Edit 3box Profile' : 'View 3box Profile'}
              </a>
              {isLoggedInUser && (
                <>
                  <a className="profileButton" onClick={() => setSelected(0)}>
                    My Files for Sale
                  </a>
                  <a className="profileButton" onClick={() => setSelected(1)}>
                    Files I've Bought
                  </a>
                  <a className="profileButton" onClick={() => setSelected(2)}>
                    My Subscribers
                  </a>
                  <a className="profileButton" onClick={() => setSelected(3)}>
                    My Subscriptions
                  </a>
                  <a className="profileButton" onClick={() => setSelected(4)}>
