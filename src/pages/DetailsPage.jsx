    ...
    async function getMetadata() {
      ...
      if (
        ...
        let isSubscriber = file.suscriptions.some(
          subscription => subscription.seller.address === file.seller.address,
        );
      }
    }
      ...
      {file && file.metadata && (
        <div className="fileItem" key={file.metadataHash}>
          <div className="detailsLeftBar">
            ...
            {isSeller && <div className="sellerButton">You sold this file</div>}
          </div>
          <div className="detailsRightBar">
            ...
            <span className="filePrice">{`${(file.price / 10 ** 18).toFixed(
              2,
            )} ${getTokenSymbol(file.priceAsset)}`}</span>
            ...
            <span className="fileDescription">{file.metadata.description}</span>
            {location && location != 'undefined' && (
              <>
                <span className="label">local file location</span>
                <span className="fileDescription">{location}</span>
              </>
            )}
          </div>
        </div>
